 ...
 * Switchinator
 * 
 * Avr project to switch things on and off at a schedule.  Uses an AVR chip, a nrf24l01 radio
 * and various switches.  See https://oshpark.com/shared_projects/ZR9ot1LA (hopefully.)
 */
...
// Adding a flag so we can just retrieve the value of an input without getting anything else.
static unsigned char onlyGetInput = 0;
static unsigned int ourInputValue = 0;

// Adding a flag so we can not have switches fire based on inputs (like overriding a motion sensor if
// it is daytime
static unsigned char doNotFireInput = 0;
...
void switchTime(char * commandReceived) {
    char tempReallyLongString[] = "0000000";
	statusMsg[0] = 0;
	int switchNumber = getInt(commandReceived, 3, 2);
    if (switchNumber >= NUM_SWITCHES) {
        fail(1);
		return;
	}
	
	returnHex(weeklySeconds, tempReallyLongString);
	strcat(statusMsg,"cur time:");
	strcat(statusMsg,tempReallyLongString);
	returnHex(switchStatus[switchNumber],tempReallyLongString);
	strcat(statusMsg,", sw time:");
	strcat(statusMsg,tempReallyLongString);
	sendMessage(statusMsg);
}

// Sometimes we just want to know what the inputs are.
void returnInput(char * commandReceived) {
	int inputNumber = getInt(commandReceived, 3, 2);
	char tempReallyLongString[] = "00000000";
	onlyGetInput = 1;
	ourInputValue = 0;
	...
	returnHex(ourInputValue,tempReallyLongString);
	strcat(statusMsg,tempReallyLongString);
	sendMessage(statusMsg);
}
...
void setAnalogInput(char * commandReceived) {
    ...
    // Set prescaler 1/128. why not... ;-)
    ADCSRA |= (1 << ADPS2) | (1 << ADPS1) | (1 << ADPS0)|(1 << ADEN);
	// Use vcc for reference voltage
	ADMUX = (1 << REFS0);
...
void getInput(int inputNumber) {
    ...
    if (low != 255 && high != 255) {
        ...
        temp = outputNum - 32;
        temp = temp / 2;
        // temp is now the portC pin number.
        ...
        ADCSRA |= (1 << ADSC); // turn on ADC and start a conversion
        ...
        ADMUX &= ~15; // reset so we are reading no pins


        //if we are only running the input to return the value
        if(onlyGetInput == 1) {
			onlyGetInput = 0;
			ourInputValue = temp;
			return;
		...
		//if something is overriding the input switch - make sure we can use the
		//whichRGB setting also
		if(doNotFireInput == 1  && (whichRGB & 8) ==0)
			return;
        
        /*
         * So I'm thinking if I want to override the input switches I can use the whichRGB
         * setting since it is always <=7.  So if whichRGB & 8 then we set or unset the
         * doNotFireInput setting.
         * TODO: you are here.
		 * static unsigned char doNotFireInput = 0;
         */
        ...
        if (temp >= low && temp <= (high + 1)) {
            ...
            // see if we are checking to override the inputs
            if(whichRGB & 8) {
				doNotFireInput = 1;
				return;
			}
        ...
        // sometimes we only want to know the value
        if(onlyGetInput == 1) {
			onlyGetInput = 0;
			ourInputValue = yeaOurInputIsOn;
			return;
		}
		
		//if something is overriding the input switch
		if(doNotFireInput == 1) 
			return;
