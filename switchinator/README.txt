...
PE:##ddssssddddswswswswPP - programEdit - directly edits a program and assumes that you know what you are doing
...
AI:##PpLLLHHH?##DuraPOw - Analog input - ## input num, port/pin, LLL%,HHH%,
			? = (s)witch or (p)rogram, ## num, dur/poll time in seconds,
			whichRGB or 8 for override - which rgb is a mask if we are trying to do RGB based on the value
			of the analog input or 8 if this analog input overrides the input of others like a light sensor
			that stops a motion sensor input to fire
DI:##Ppx?##DuraPO - Digital input. x=H/L for what activates switch
CI xx - Clear Input
RI:xx - Receive input - Displays the current status of the input.
GS - General Status
